...
void MainFrame::init_tabpanel()
{
   ...
   // BBS
    ...
    m_tabpanel->Bind(wxEVT_NOTEBOOK_PAGE_CHANGING, [this](wxBookCtrlEvent& e) {
        ...
        const bool is_dlp = wxGetApp().preset_bundle != nullptr &&
            wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology() == ptSLA;
        if (is_dlp && new_sel == tpPreview && old_sel != tpPreview) {
            // The built-in Preview page consumes FDM G-code / native SLA
            // results. DLP uses independent raster layers, so show its safe,
            // image-based layer viewer instead.
            e.Veto();
            CallAfter([this]() {
                if (m_plater)
                    m_plater->preview_dlp_images();
            });
            return;
        }
...
void MainFrame::create_preset_tabs()
{
    ...
    /* DLP printers use the existing SLA process and material configuration model. */
    ...
    m_slice_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event)
        {
            const bool is_dlp = wxGetApp().preset_bundle != nullptr &&
                wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology() == ptSLA;
            if (is_dlp) {
                // DLP layer masks are generated by the independent engine from
                // the Output command. Never start the native SLA worker here:
                // it duplicates the work and races model-editing gizmos.
                m_plater->reset_check_status();
                m_plater->set_slice_from_slice_btn(true);
                m_plater->exit_gizmo();
                m_plater->update(false, false);
                if (!m_plater->preview_dlp_images())
                    return;
                m_slice_btn->SetLabel(_L("Sliced"));
                m_slice_btn->Disable();
                m_slice_enable = false;
                m_print_enable = true;
                m_print_btn->Enable();
                Layout();
                m_plater->show_status_message(_u8L("DLP plate slicing completed. Click Output to save layer images."));
                return;
            }
...
bool MainFrame::get_enable_slice_status()
{
    ...
    if (is_dlp) {
        // SLA/DLP background processing may already be active while editing.
        // Enable the explicit slice command whenever a printable model is on
        // the current plate.
        return current_plate != nullptr && current_plate->can_slice() &&
               !m_plater->model().objects.empty();
    }
...
void MainFrame::init_menubar_as_editor()
{
    ...
    wxMenu* fileMenu = new wxMenu;
    {
        ...
        append_menu_item(export_menu, wxID_ANY, _L("Export all objects as STLs") + dots, _L("Export all objects as STLs"),
            ...
            [this](){return can_export_model(); }, this);
        append_menu_item(export_menu, wxID_ANY, _L("Export DLP Images") + dots, _L("Slice the current model and export DLP layer PNG images"),
            [this](wxCommandEvent&) { if (m_plater) m_plater->export_dlp_images(); }, "menu_export_sliced_file", nullptr,
            [this](){return can_export_model(); }, this);
...
void MainFrame::set_dlp_button_mode(bool enabled)
{
    if (enabled) {
        m_slice_select = eSlicePlate;
        m_print_select = eExportSlicedFile;
        m_slice_btn->SetLabel(_L("Slice plate"));
        m_print_btn->SetLabel(_L("Output"));
        m_slice_option_btn->Disable();
        m_print_option_btn->Disable();
    } else {
        m_slice_option_btn->Enable();
        m_print_option_btn->Enable();
        m_slice_select = eSlicePlate;
